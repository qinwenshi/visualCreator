...
!function (exports){
  ...
  raw.parser = function (delimiter) {
    var data = [];
    function parse_internal(string, parseCompleteFn) {
        var rows = [];
        if (!string || string.length === 0) return performCompletion(rows);
        var data = [];
        var completeFn = function(){
            if (arguments[0] && arguments[0].data)
              rows = arguments[0].data;
            performCompletion(rows);
        };

        var performCompletion = function(rows){
          if(!!parseCompleteFn)
            parseCompleteFn(rows);
        };
        var buildConfig = function(){
          var getLineEnding = function() {
            return "";
          };

          var errorFn = function(error, file){
    ...
    parser.metadata = function(data) {
      //return d3.entries(sniff(parser(string))).map(function (d){
      return d3.entries(sniff(data)).map(function (d){
  ...
  raw.model = function () { 
    ...
    model.instruction = function() {
      return dimensions.values()
        .filter(function(d){ return d.required() > d.value.length; })
        .map(function(d){ var v = d.required() - d.value.length > 1 ? '维度' : '维度'; return '<b>'+d.title() + "</b> 还需要 " + (d.required() - d.value.length) + " 个 " + v;  })
        .join(". ")
    }
